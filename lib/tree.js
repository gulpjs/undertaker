...
function tree(opts, transformer) {
  ...
  if (!opts.deep) {
    maxDepth = 1;
    if (transformer == null) {
      transformer = transformLabelOnly;
    }
  } else if (opts.depth == null || opts.depth > 0) {
    maxDepth = opts.depth;
    if (transformer == null) {
      transformer = transformThroughPass;
    }
  } else {
    maxDepth = 0;
    if (transformer == null) {
...
function transformLabelOnly(node/*, metaTree, depth, maxDepth*/) {
  return node.label;
}

function transformThroughPass(node/*, metaTree, depth, maxDepth*/) {
  return node;
}
